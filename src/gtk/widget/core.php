...
class core {
    ...
    public function set_size_request(int $width, int $height){
        $this->ffi->gtk_widget_set_size_request($this->cdata_instance,$width,$height);
    }
     ...
     */
    public function get_phpOBJ($cdata){
        $widget = \gtk\core::cast2Widget($cdata);
        $gtype = $this->G_OBJECT_TYPE($widget);
        $gtype_name = $this->ffi->g_type_name($gtype);
        // Convert it to PHP-GTK class
        $phpgtk_name = "\\gtk\\widget\\" . strtolower(substr($gtype_name, 3));
        $reflection = new \ReflectionClass($phpgtk_name);
        $phpOBJ = $reflection->newInstanceWithoutConstructor();
